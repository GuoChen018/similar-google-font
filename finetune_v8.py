...
app = modal.App("font-finetune-v8")
...
def finetune():
    ...
    for font_path in tqdm(font_files, desc="Rendering glyphs + text"):
        ...
        for glyph in GLYPHS:
            ...
            if not os.path.exists(out_path):
                img = render_text_image(font_path, glyph)
                if img is None:
                    continue
                img.save(out_path)
    ...
    for epoch in range(EPOCHS):
        ...
        if r5 > best_recall5:
            best_recall5 = r5
            patience_counter = 0
            torch.save(model.state_dict(), "/data/font_v8_best_full.pt")
            volume.commit()
        ...
        torch.save(model.state_dict(), f"/data/checkpoint_v8_epoch{epoch+1}.pt")
        volume.commit()

    # ── 14. Merge LoRA and save inference model ───────────────────────────────
    if not os.path.exists("/data/font_v8_best_full.pt"):
        print("ERROR: No best model was saved. Saving current model as fallback.")
        torch.save(model.state_dict(), "/data/font_v8_best_full.pt")
        volume.commit()

    print("Merging LoRA weights for inference...")
    best_state = torch.load("/data/font_v8_best_full.pt", map_location=device)
    ...
    torch.save(inference_model.state_dict(), "/data/font_embedder_v8.pt")
    volume.commit()
    print(f"Inference model saved to /data/font_embedder_v8.pt")
