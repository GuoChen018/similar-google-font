...
app = modal.App("font-finetune-v5")
...
def finetune():
    ...
    TEXT_STRINGS = ["Hamburgevons", "Aa Bb Gg", "Quick fox"]

    def render_text_image(font_path, text, size=224):
        """Render a text string centered in a square image, auto-sizing the font."""
    ...
    # ── 6. Dataset + family-aware PK batch sampler ──────────────────────────
    # Build font-to-family-id mapping
    family_name_to_id = {}
    font_family_id_map = {}
    for f in font_files_list:
        fam = fontfile_to_family.get(f, f)
    ...
    class FontDataset(Dataset):
        def __init__(self, fonts, fontfile_to_paths, fontfile_tag_vectors,
                     font_family_id_map, transform):
            self.fonts = fonts
        ...
        def __getitem__(self, idx):
            ...
            return self.transform(img), idx, tag_vec, family_id

    class FamilyPKSampler(Sampler):
        """P font families x K samples per family."""
        def __init__(self, fonts, fontfile_to_family, p=32, k=2,
    ...
    def family_identity_supcon(embeddings, family_ids, temperature=0.07):
        """Primary loss: pull same-family embeddings together, push different apart."""
        n = len(embeddings)
    ...
    def tag_similarity_supcon(embeddings, tag_vectors, temperature=0.07):
        """Secondary loss: group fonts with similar tags for cross-family similarity."""
    ...
    train_dataset = FontDataset(train_fonts, fontfile_to_paths, fontfile_tag_vectors,
                                font_family_id_map, augment)
    val_dataset = FontDataset(val_fonts, fontfile_to_paths, fontfile_tag_vectors,
                              font_family_id_map, clean)

    train_sampler = FamilyPKSampler(
        train_fonts, fontfile_to_family,
        p=32, k=2, batches_per_epoch=1500
    )
    val_sampler = FamilyPKSampler(
        val_fonts, fontfile_to_family,
        p=32, k=2, batches_per_epoch=200
    )
    ...
    # Camera augmentation for Recall@k queries (realistic camera conditions)
    camera_query_aug = transforms.Compose([
        RandomBackground(),
        transforms.Resize((224, 224)),
    ...
    # Pre-render recall query base images for val fonts (rendered once, augmented each epoch)
    ...
    def evaluate(model, fonts, fontfile_to_paths, fontfile_tag_vectors):
        ...
        # ── Build clean font index (same as inference) ──
        all_paths = []
        all_font_idx = []
        ...
        # ── cat_acc + tag@5 (existing metrics) ──
        ...
        # ── Recall@k: synthetic camera query → font retrieval ──
        # Group fonts by family so we match at family level
        font_to_family_idx = {}
        family_names = []
        for fidx, font_name in enumerate(fonts):
        ...
        # Batch recall queries for GPU efficiency
        query_tensors = []
        query_family_ids = []
        for fidx, font_name in enumerate(fonts):
            font_path = fontfile_to_fontpath.get(font_name)
            ...
            for base_img in recall_base_images.get(font_name, []):
                query_tensors.append(camera_query_aug(base_img.copy()))
        ...
        if query_tensors:
            ...
            for i in range(0, len(query_batch), 128):
                ...
                query_embs.append(model.embed(chunk).cpu())
    ...
    for epoch in range(EPOCHS):
        ...
        for images, indices, tag_vecs, fam_ids in tqdm(train_loader, desc=f"Epoch {epoch+1}/{EPOCHS} train"):
            images = images.to(device)
            tag_vecs = tag_vecs.to(device)
            fam_ids_list = fam_ids.tolist()

            with autocast("cuda"):
                embeddings, tag_logits = model(pixel_values=images)
                loss_family = family_identity_supcon(embeddings, fam_ids_list)
                loss_tag = tag_similarity_supcon(embeddings, tag_vecs)
                loss_bce = multilabel_tag_bce(tag_logits, tag_vecs)
                loss = loss_family + 0.3 * loss_tag + 0.2 * loss_bce
        ...
        val_batches = 0
        with torch.no_grad():
            for images, indices, tag_vecs, fam_ids in tqdm(val_loader, desc=f"Epoch {epoch+1}/{EPOCHS} val"):
                images = images.to(device)
                tag_vecs = tag_vecs.to(device)
                fam_ids_list = fam_ids.tolist()
                with autocast("cuda"):
                    ...
                    lt = tag_similarity_supcon(embeddings, tag_vecs)
                    lb = multilabel_tag_bce(tag_logits, tag_vecs)
                val_family += lf.item()
                val_tag += lt.item()
                val_bce += lb.item()
                val_batches += 1
        ...
        avg_val_tag = val_tag / max(val_batches, 1)
        avg_val_bce = val_bce / max(val_batches, 1)

        metrics = evaluate(model, val_fonts, fontfile_to_paths, fontfile_tag_vectors)
        lr = optimizer.param_groups[0]['lr']
        print(f"Epoch {epoch+1} — fam: {avg_family:.4f} tag: {avg_tag:.4f} bce: {avg_bce:.4f} | "
              f"v_fam: {avg_val_family:.4f} v_tag: {avg_val_tag:.4f} | "
              f"R@1: {metrics['R@1']:.4f} R@5: {metrics['R@5']:.4f} R@10: {metrics['R@10']:.4f} | "
              f"cat: {metrics['cat_acc']:.4f} tag@5: {metrics['tag_overlap@5']:.4f} | "
              f"lr: {lr:.2e}")
        ...
        if r5 > best_recall5:
            best_recall5 = r5
            patience_counter = 0
            torch.save(model.state_dict(), "/data/font_v5_best_full.pt")
            volume.commit()
        ...
        torch.save(model.state_dict(), f"/data/checkpoint_v5_epoch{epoch+1}.pt")
        volume.commit()

    # ── 13. Merge LoRA and save inference model ───────────────────────────────
    if not os.path.exists("/data/font_v5_best_full.pt"):
        print("ERROR: No best model was saved during training. Saving current model as fallback.")
        torch.save(model.state_dict(), "/data/font_v5_best_full.pt")
        volume.commit()

    print("Merging LoRA weights for inference...")
    best_state = torch.load("/data/font_v5_best_full.pt", map_location=device)
    ...
    torch.save(inference_model.state_dict(), "/data/font_embedder_v5.pt")
    volume.commit()
    print(f"Inference model saved to /data/font_embedder_v5.pt")
