...
app = modal.App("font-finetune-v4")
...
def finetune():
    ...
    # Heavy camera augmentation for Recall@k queries (harder than training aug)
    camera_query_aug = transforms.Compose([
        RandomBackground(),
        transforms.Resize((224, 224)),
        transforms.RandomPerspective(distortion_scale=0.25, p=0.5),
        transforms.RandomRotation(8),
        transforms.ColorJitter(brightness=0.8, contrast=0.8, saturation=0.4, hue=0.08),
        transforms.GaussianBlur(kernel_size=7, sigma=(0.5, 4.0)),
        transforms.RandomAdjustSharpness(sharpness_factor=2, p=0.3),
        transforms.RandomInvert(p=0.15),
        transforms.ToTensor(),
        GaussianNoise(max_sigma=0.08),
    ...
    for epoch in range(EPOCHS):
        ...
        if r5 > best_recall5:
            best_recall5 = r5
            patience_counter = 0
            torch.save(model.state_dict(), "/data/font_v4_best_full.pt")
            volume.commit()
        ...
        torch.save(model.state_dict(), f"/data/checkpoint_v4_epoch{epoch+1}.pt")
        volume.commit()

    # ── 13. Merge LoRA and save inference model ───────────────────────────────
    if not os.path.exists("/data/font_v4_best_full.pt"):
    ...
    best_state = torch.load("/data/font_v4_best_full.pt", map_location=device)
    ...
    torch.save(inference_model.state_dict(), "/data/font_embedder_v4.pt")
    volume.commit()
    print(f"Inference model saved to /data/font_embedder_v4.pt")
